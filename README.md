...
**内置插件（推荐，零配置）**：常用插件（如「半透明聊天背景」）已经直接打包进 `index.html`，**打开网页即自动启用**，无需任何操作、无需上传额外文件或填地址。打开设置即可看到其控件。

**导入式插件（像 MC mod 一样，选文件即用）**：在「插件管理」点「选择文件」挑一个本地 `.js` 插件，再点「导入」即可。插件会被读取并立即安装、自动启用，且**源码持久化在浏览器，刷新或重开网页后自动加载**（相当于 MC 的 mods 文件夹）。无需粘贴地址、无需勾选。

**自定义插件的其他加载方式**：如果你自己写了插件 `.js` 文件，还有两种方式：

- **地址安装**：在输入框粘贴插件的 `.js` 地址，点「安装」即可加载（脚本需自行调用 `AgentConsole.registerPlugin`）。
- **URL 自动加载**：访问 `你的站点/?plugin=地址`，页面打开时自动加载。
- **manifest 自动读取**：在站点根目录放一个 `plugins/manifest.json`，内容为插件 `.js` 地址的字符串数组，如 `["plugins/example-plugin.js"]`，网页打开时会自动逐个加载。

无论哪种方式，加载后的插件都会出现在列表中，你可以：

- **启用 / 禁用**：勾选复选框即启用、取消即禁用；禁用后该插件的 `onBeforeSend` 等钩子不再生效。
- **卸载**：点「卸载」移除（导入式插件卸载后会一并清除本地持久化）。

> 以「半透明聊天背景」为例：网页打开即生效，设置面板底部会多出「聊天背景」控件，可**选择本地图片**或**粘贴图片链接**作为半透明背景，并用滑块调节透明度；状态会保存，刷新后自动恢复。
