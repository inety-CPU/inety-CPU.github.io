...
**内置插件（推荐，零配置）**：常用插件（如「半透明聊天背景」）已经直接打包进 `index.html`，打开设置后就会出现在列表里，**只要勾选复选框即可启用**，不需要任何安装、不需要上传额外文件、也不需要填地址。

**自定义插件**：如果你自己写了插件 `.js` 文件，有两种加载方式：
...
- **启用 / 禁用**：勾选复选框即启用、取消即禁用；禁用后该插件的 `onBeforeSend` 等钩子不再生效。状态保存在本地 `localStorage`，刷新后保留。
- **卸载**：点「卸载」从当前会话移除该插件。

> 以「半透明聊天背景」为例：在列表中勾选启用后，设置面板底部会多出「聊天背景」控件，可**选择本地图片**或**粘贴图片链接**作为半透明背景，并用滑块调节透明度；状态会保存，刷新后自动恢复。
